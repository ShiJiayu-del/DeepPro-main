...
def validate_payload_metrics(
    payload, curve, context, expected_inference_amp=True,
    validate_pixel_diagnostics=True,
):
    """Recompute aggregate metrics from the validated integer counts.

    ``validate_pixel_diagnostics`` defaults to the historical strict behavior.
    Paper-aligned protocols can disable that auxiliary check so pixel metrics
    present in an older payload remain inert metadata rather than evaluation or
    ...
    if payload.get('inference_amp') is not expected_inference_amp:
        _context_error(
            context,
            'inference_amp must be %s' % expected_inference_amp,
        )
    _require_int_equal(
    ...
    if validate_pixel_diagnostics:
        ...
        for field in pixel_fields:
            ...
            if value < 0.0 or value > 1.0:
                _context_error(
                    context, '%s must be finite and in [0, 1]' % field,
                )
