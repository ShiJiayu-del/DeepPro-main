...
为验证优化后的 `ShootingRules` 与官方逐阈值实现是否一致，本次审计曾对已停止、不得进入
结果的 C1 临时 checkpoint 在本地 test20 做一次只读计数对拍；27 个阈值的计数一致。
该结果未保存为候选指标、未参与 checkpoint 或结构选择，也不改变当前正式重跑仅使用
internal-val16 的规则。该 test20 在项目历史实验中本就已有暴露，不能描述为首次未见测试集。
